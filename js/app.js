...
function fetchExamples(word) {
  ...
  var exampleHeader = DOM.examplesBoxHeaderElement;
  var exampleBody = DOM.examplesBodyElement;
  ...
  let exampleBodyOutput = " ";

  if (ex1) exampleBodyOutput += `<p>• ${ex1}</p>`;
  if (ex2) exampleBodyOutput += `<p>• ${ex2}</p>`;
  if (ex3) exampleBodyOutput += `<p>• ${ex3}</p>`;
  if (ex4) exampleBodyOutput += `<p>• ${ex4}</p>`;

  exampleBox.style.display = "block";
  exampleHeader.style.display = "block";
  exampleBody.style.display = "block";
  exampleBody.innerHTML = exampleBodyOutput;
...
function autoFitText() {
  ...
  options.forEach(span => {
    ...
    while (span.scrollHeight > span.parentElement.clientHeight && fontSize > 11) {
      fontSize--;
      span.style.fontSize = fontSize + "px";
    }
  });
...
function validate() {
  ...
  // DOM.nextBtn.style.backgroundColor = "#2563eb";
...
function addToRevisionList(word, meaning) {
  var table = DOM.revisionTableBody;
...
function addWordToList(Word, Meaning) {
  ...
  tooltip.innerHTML = `<div id="tooltipHeader">
...
function clearRevisionList() {
  DOM.revisionTableBody.innerHTML = "";
}
...
document.addEventListener("DOMContentLoaded", function () {
  DOM = {
    ...
    examplesBoxHeaderElement: document.getElementById("examplesBoxHeader"),
    examplesBodyElement: document.getElementById("examplesBody"),
    refreshBtn: document.getElementById("refreshBtn"),
    reportBtn: document.getElementById("reportBtn"),
    revisionTable: document.getElementById("RevisionList"),
    revisionTableBody: document.getElementById("RevisionListBody"),
  ...
  const table = DOM.revisionTableBody;
...
function submitAnswer() {
  ...
  fetch("https://script.google.com/macros/s/AKfycbzz8e-a0Bwxt-6o3vjn74wn1IOHGQZNPZniAxFar73iwZ0y1JGsF0pA8eLWY6UOONez/exec", {
    method: "POST",
...
function loadNextQuestion() {
  ...
  document.getElementById("applyBubbleWrapper").style.position = "relative";
  document.getElementById("applyInteractionCard").style.marginTop = "25px";
  ...
  // ✅ Center the middle bubble of the round
  const bubbleIndex = currentRoundIndex * 5 + 2;
  ...
  document.getElementById("applyBubbleWrapper").style.position = "absolute";
  document.getElementById("applyInteractionCard").style.marginTop = "205px";
...
function startApplySession() {
  ...
  script.src =
    "https://script.google.com/macros/s/AKfycbzz8e-a0Bwxt-6o3vjn74wn1IOHGQZNPZniAxFar73iwZ0y1JGsF0pA8eLWY6UOONez/exec" +
...
function updateBubbleVisibility() {
  ...
  const start = currentRoundIndex * 5;
  const end = start + 5;
...
function getRoundForIndex(index) {
  if (index <= 4) return 0;        // Round 1 → 0–4
  if (index <= 7) return 1;        // Round 2 → 3–7
  if (index <= 10) return 2;       // Round 3 → 6–10
  if (index <= 13) return 3;       // Round 4 → 9–13
  if (index <= 16) return 4;       // Round 5 → 12–16
}

function loadRoundWords() {

  // 🔥 ADD HERE (FIRST LINE)
  if (!sessionWordPool || sessionWordPool.length < 5) {
    console.error("Not enough words for session", sessionWordPool);
    return;
  }
  ...
  sessionWordPool.forEach((wordObj, index) => {

    const bubble = document.createElement("div");
    bubble.className = "apply-bubble";
    const meaningHTML = wordObj.meaning.trim()
...
function updateVisibleWords() {
  ...
  // remove old ids
  bubbles.forEach(b => b.removeAttribute("id"));

  // assign ids only to visible ones
  for (let i = 0; i < 5; i++) {
    const index = start + i;
    if (bubbles[index]) {
      bubbles[index].id = "visible-" + i;
    }
  }
...
async function loginUser() {
  ...
  try {
    ...
    const res = await fetch("https://script.google.com/macros/s/AKfycbzz8e-a0Bwxt-6o3vjn74wn1IOHGQZNPZniAxFar73iwZ0y1JGsF0pA8eLWY6UOONez/exec", {
      method: "POST",
      body: JSON.stringify({
        action: "login",
        user: userInput,
...
function refreshLevelProgress() {
  const userId = localStorage.getItem("loggedInUser");
  fetch("https://script.google.com/macros/s/AKfycbzz8e-a0Bwxt-6o3vjn74wn1IOHGQZNPZniAxFar73iwZ0y1JGsF0pA8eLWY6UOONez/exec" + "?action=getUserProgress&user_id=" + userId)
    .then(res => res.json())
    .then(data => {
...
function updateLevelUI(level, totalMastered, masteredInLevel, levelTarget) {
  ...
  masteredInLevel = Number(masteredInLevel) || 0;
  levelTarget = Number(levelTarget) || 0;

  const remaining = Math.max(0, levelTarget - masteredInLevel);
  ...
  const percent = levelTarget > 0
    ? (masteredInLevel / levelTarget) * 100
    : 0;
...
async function loadDashboard() {
  ...
  try {
    const res = await fetch(
      "https://script.google.com/macros/s/AKfycbzz8e-a0Bwxt-6o3vjn74wn1IOHGQZNPZniAxFar73iwZ0y1JGsF0pA8eLWY6UOONez/exec" + "?action=getUserDashboard&user_id=" + userId
    );
...
<div class="pageJump">
<input type="number" id="pageJumpInput" min="1" max="${totalPages}" placeholder="page.no">
...
async function loadInsights() {
  const userId = localStorage.getItem("loggedInUser");
  const res = await fetch(
    "https://script.google.com/macros/s/AKfycbzz8e-a0Bwxt-6o3vjn74wn1IOHGQZNPZniAxFar73iwZ0y1JGsF0pA8eLWY6UOONez/exec" + "?action=getUserInsights&user_id=" + userId
  );
