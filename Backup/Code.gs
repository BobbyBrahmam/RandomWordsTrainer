...
function getEffectiveLevelLimits() {
  ...
  const raw = {
    1: sheet.getRange("C2").getValue(),
    2: sheet.getRange("C3").getValue(),
    3: sheet.getRange("C4").getValue(),
    4: sheet.getRange("C5").getValue(),
    5: sheet.getRange("C6").getValue()
  };

  const limits = {};

  for (let lvl = 1; lvl <= 5; lvl++) {
    limits[lvl] = Math.min(raw[lvl], 200);
  }
...
function getUserProgress(userId) {
  ...
  const masteredInLevel = mastered - cumulative;

  /* ✅ target for current level */
  const target = limits[level];
