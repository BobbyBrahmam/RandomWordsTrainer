...
function updateReport() {
  ...
  if (accuracy >= 100) {
    ...
    offset = circumference - (accuracy / 100) * circumference;
  }
...
function openReportModal() {
  ...
  updateReport(); // make sure stats are updated
}
...
const shortDrakToggleBtn = document.getElementById("onStripDarkToggle");
...
function checkUsedWords() {
  ...
  const wordsInText = cleanText.split(/\s+/).map(w => stem(w));
  const bubbles = document.querySelectorAll(".apply-bubble");
  bubbles.forEach((bubble) => {
    const word = bubble
      .querySelector(".apply-bubble-word")
      .innerText;
    const bubbleStem = stem(normalize(word));
    const prefix = bubbleStem.slice(0, Math.min(4, bubbleStem.length));
    const match = wordsInText.some(w => w.startsWith(prefix));
    if (match) {
...
function highlightWordsInText(text, words) {
  ...
  // 🔹 Handle Gemini correction array automatically
...
Suggestion: ${c.suggestion}`
  ...
  const highlighted = tokens.map(token => {
    ...
    for (let word of words) {
      ...
      const prefix = cleanWord.slice(0, Math.min(4, cleanWord.length));
...
function submitAnswer() {
  ...
  fetch("https://script.google.com/macros/s/AKfycbyc98rhS6khxwKT_trQ5tTIukhj7AZ-vUm_nklcPxXgYdv4u23g2Z5WYvrNVi6Mm98A/exec", {
    method: "POST",
    ...
    .then(text => {
      ...
      const highlightedSuggestion =
        highlightWordsInText(ai.suggested_correction, currentApplyWords);
...
function startApplySession() {
  ...
  script.src =
    "https://script.google.com/macros/s/AKfycbyc98rhS6khxwKT_trQ5tTIukhj7AZ-vUm_nklcPxXgYdv4u23g2Z5WYvrNVi6Mm98A/exec" +
...
async function loginUser() {
  ...
  try {
    ...
    const res = await fetch("https://script.google.com/macros/s/AKfycbyc98rhS6khxwKT_trQ5tTIukhj7AZ-vUm_nklcPxXgYdv4u23g2Z5WYvrNVi6Mm98A/exec", {
      method: "POST",
      body: JSON.stringify({
        action: "login",
        user: userInput,
...
function refreshLevelProgress() {
  const userId = localStorage.getItem("loggedInUser");
  fetch("https://script.google.com/macros/s/AKfycbyc98rhS6khxwKT_trQ5tTIukhj7AZ-vUm_nklcPxXgYdv4u23g2Z5WYvrNVi6Mm98A/exec" + "?action=getUserProgress&user_id=" + userId)
    .then(res => res.json())
    .then(data => {
      updateLevelUI(
        data.level,
...
function endSession() {
  ...
  fetch("https://script.google.com/macros/s/AKfycbyc98rhS6khxwKT_trQ5tTIukhj7AZ-vUm_nklcPxXgYdv4u23g2Z5WYvrNVi6Mm98A/exec", {
    ...
    body: JSON.stringify({
      ...
      session_accuracy: calculateSessionAccuracy(),
...
function updateLevelUI(level, mastered, target) {

  let levelStart = 0;

  if (level === 2) levelStart = 80;
  if (level === 3) levelStart = 200;
  if (level === 4) levelStart = 400;
  if (level === 5) levelStart = 700;

  const levelMastered = mastered - levelStart;
  const levelTarget = target - levelStart;
  const remaining = levelTarget - levelMastered;
  ...
  document.getElementById("masteredWords").textContent = levelMastered;
  document.getElementById("targetWords").textContent = levelTarget;

  document.getElementById("progressDone").textContent = levelMastered;
  document.getElementById("progressRemaining").textContent = remaining;

  const percent = (levelMastered / levelTarget) * 100;
  document.getElementById("progressFill").style.width = percent + "%";

  /* TOTAL MASTERED (new stat) */

  document.getElementById("totalMasteredWords").textContent = mastered;
}
...
async function loadDashboard() {
  ...
  try {
    const res = await fetch(
      "https://script.google.com/macros/s/AKfycbyc98rhS6khxwKT_trQ5tTIukhj7AZ-vUm_nklcPxXgYdv4u23g2Z5WYvrNVi6Mm98A/exec" + "?action=getUserDashboard&user_id=" + userId
    );
...
function searchWordTable() {
  ...
  const rows =
    document.querySelectorAll("#wordTableBody tr");
  rows.forEach(row => {
    const word =
